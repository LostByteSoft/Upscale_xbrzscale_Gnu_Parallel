...
echo -------------------------========================-------------------------
	...
	echo
	echo "Software name: mogrify -resize 150% -format jpg"
	echo 
	echo "File name : Convert All Folder to JPG +50 (cluster).sh"
	echo
	echo "What it does ? Convert All Folder to JPG +50 % size (cluster) using GNU Parallel lan cpu."
	echo
...
part=$((part+1))
	echo "-------------------------===== Section $part =====-------------------------"
echo Mogrify by lan cores...

	cd "$file" && ls *.* | parallel ""$cluster"" --workdir /dev/shm --eta --transfer 'mogrify -resize 150% -format jpg -quality 95 {}'
...
if [ "$debug" -eq "1" ]; then
	...
	echo "ReGet files one server at a time... (Slower , No ReGet if no files , no overload lan)"
		...
		nummaster=0
		echo
		nummaster="$(ssh master@192.168.0.75 'ls 2>/dev/null /dev/shm/*.jpg | wc -l && exit ; exec bash --login')"
			echo Files on Master : $master
			if [ "$nummaster" -ge 1 ]; then
			echo "Files are UpLoaded !"
			rsync -ahvzP master@192.168.0.75:/dev/shm/*.jpg """$file""/$subfolder" 2> /dev/null	## work
		fi
		echo
		numserver="$(ssh server@192.168.0.25 'ls 2>/dev/null /dev/shm/*.jpg | wc -l && exit ; exec bash --login')"
			echo Files on Server : $server
			if [ "$numserver" -ge 1 ]; then
			echo "Files are UpLoaded !"
			rsync -ahvzP server@192.168.0.25:/dev/shm/*.jpg """$file""/$subfolder"			## work
		fi
		echo
		numasus="$(ssh asus@192.168.0.30 'ls 2>/dev/null /dev/shm/*.jpg | wc -l && exit ; exec bash --login')"
			echo Files on Asus : $asus
			if [ "$numasus" -ge 1 ]; then
			echo "Files are UpLoaded !"
			rsync -ahvzP asus@192.168.0.30:/dev/shm/*.jpg """$file""/$subfolder" 			## work
		fi
		echo
		numacer="$(ssh acer@192.168.0.40 'ls 2>/dev/null /dev/shm/*.jpg | wc -l && exit ; exec bash --login')"
			echo Files on Acer : $acer
			if [ "$numacer" -ge 1 ]; then
			echo "Files are UpLoaded !"
			rsync -ahvzP acer@192.168.0.40:/dev/shm/*.jpg """$file""/$subfolder"			## work
		fi
