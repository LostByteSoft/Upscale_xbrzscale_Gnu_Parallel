...
#!/usr/bin/ffmpeg
	...
	version=2026-06-30_Tuesday_18:05:09
...
echo Finding files...
	...
	if [ "$debugcore" -eq "0" ]; then
			printf '\033[8;25;140t'		## will resize the window.
		else
			printf '\033[8;45;140t'		## will resize the window.
		fi
...
while IFS= read -r img; do
    ...
    if [ -f "$img" ]; then
        ...
        # Validate width
        if ! [[ "$width" =~ ^[0-9]+$ ]]; then
            echo "Warning: Could not get width for $img" >&2
            continue
        fi
	...
	echo "XbrzScale | Value: $entry2 | Number: $conv | Width: $width | File: $img" | tee -a "/dev/shm/xbrscale.txt"
	xbrzscale "$entry2" "$img" "${img%.*}_UpScale.jpg" 2> /dev/null
    else
        echo "Warning: File not found - $img" >&2
    fi
		...
		if zenity --no-wrap --question --text="Do you want to reconvert to save space ?\n\n\t(Yes or No (Suggest Yes))"
			...
			echo "-------------------------===== Section $part =====-------------------------"
				...
				while IFS= read -r "line"; do
					...
					echo "Number $conv | mogrify -format jpg -resize 95% | "$line"" | tee -a "/dev/shm/xbrscale.txt"
...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "Files in folder counter / checker. Version 1,07"
	echo
	# var for section
	count1=0
	count2=0
