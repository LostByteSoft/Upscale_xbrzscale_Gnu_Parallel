...
#!/usr/bin/ffmpeg
	...
	version=2026-07-01_Wednesday_12:38:12
...
echo -------------------------========================-------------------------
echo "Select folder or filename using dialog ! Version 1,03"
	echo
...
if [ $automatic -eq 0 ]; then
		if zenity --no-wrap --question --text="Do you want to reconvert to save space ? (Yes or No (Suggest Yes))"; then
			...
			parallel -a "/dev/shm/findimg.txt" -j $entry mogrify -format jpg -resize 100% -quality 90	## jpg
			rm "/dev/shm/findimg.txt" 2> /dev/null
			...
			echo "parallel -a "/dev/shm/findimg.txt" -j $entry mogrify -format jpg -resize 100% -quality 90"
			parallel -a "/dev/shm/findimg.txt" -j $entry mogrify -format jpg -resize 100% -quality 90	## jpg
			rm "/dev/shm/findimg.txt" 2> /dev/null
			echo Conversion finish...
	fi
