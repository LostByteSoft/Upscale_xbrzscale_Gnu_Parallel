...
echo -------------------------========================-------------------------
	...
	echo 2022-11-10_Thursday_06:05:16
	echo
	...
	then
		...
		## convert to jpg or webp
			...
			do
				...
				echo Conversion number $conv
				echo "$line"
				echo convert "$line""" --define webp:lossless=true -format webp "$line"		## webp
				convert "$line""" -define webp:lossless=true -format webp "$line"		## webp
				#echo convert "$line" -format jpg -quality 100 "$line"_UpScale.jpg""
				#mogrify "$line" -format jpg -quality 100 "$line"_UpScale.jpg""
				done < "$input"
			}
		else
